...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.zoomable').forEach((el) => {
        el.addEventListener('click', (e) => {
            // Prevent any parent/link navigation and ensure the zoom always triggers.
            e.preventDefault();
            e.stopPropagation();
            const src = el.getAttribute('data-full') || el.getAttribute('src');
            openImgPreview(src, el.getAttribute('alt'));
        });
    });


    // Preview: click opens full screen
    ...
    if (previewImg) {
        previewImg.addEventListener('click', (e) => {
            ...
            if (src) openImgModal(src, previewImg.getAttribute('alt'));
        });
    }

    const previewClose = document.getElementById('img-preview-close');
    if (previewClose) previewClose.addEventListener('click', closeImgPreview);

    const preview = document.getElementById('img-preview');
    if (preview) {
        preview.addEventListener('click', (e) => {
            if (e.target === preview) closeImgPreview();
        });
    }
    ...
    // ESC closes
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') { closeImgModal(); closeImgPreview(); }
    });
});
